...
product computed through the NTT (`(f.ntt * g.ntt).nttInv`) equals the negacyclic
...
private def ntt (f : P) : NTTPolynomial := f.ntt

private def viaNTT (f g : P) : P := (ntt f * ntt g).nttInv

private instance : ToString P := ⟨fun f => toString (f.toList.map (·.val))⟩
private instance : ToString NTTPolynomial := ⟨fun f => toString (f.flatten.toList.map (·.val))⟩
