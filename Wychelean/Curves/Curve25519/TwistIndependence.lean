...
import Wychelean.Curves.Curve25519.WeierstrassScaling
...
`_scalarMul` takes the nonresidue `d` to twist by as a parameter. This module proves the choice is
immaterial:
* `_scalarMul_indep_d`, any two nonresidues compute the same function
* `scalarMul_eq`, so in particular `scalarMul`, which twists by 2, is `_scalarMul` for every one

This is Theorem 2.1 of Bernstein's Curve25519 paper (`curve25519-20060209.pdf`, proved in its
Appendix A): for `E: y² = x³ + Ax² + x` over `Fₚ` and `X₀: E(Fₚ²) → Fₚ²` the u-coordinate map, given
`n` and `q ∈ Fₚ` there is a *unique* `s ∈ Fₚ` with `X₀(nQ) = s` for every `Q ∈ E(Fₚ²)` with
`X₀(Q) = q`. The paper puts the curve and all of its twists inside `E(Fₚ²)` at once, so the choice
of twist never appears; here the twists are instead parametrised by the nonresidue `d`, and the
corresponding statement is that `_scalarMul d hd` does not depend on `d`.

The mechanism is the isomorphism of `Wychelean.Curves.Curve25519.WeierstrassScaling`. Writing
...
theorem twist_scalesTo {d d' s: BaseField} (hs: s ≠ 0) (hdd: d' = s ^ 2 * d):
    (Twist d).ScalesTo (Twist d') s where
  ne_zero := hs
  a₁ := by simp [Twist]
...
theorem uCoord_scale {W W': WeierstrassCurve BaseField} {s: BaseField} (h: W.ScalesTo W' s)
    (P: W.toAffine.Point): uCoord (Affine.Point.scale h P) = s ^ 2 * uCoord P := by
  cases P with
...
theorem uCoord_nsmul_mkTwistPoint_div {d d' u: BaseField} (hd: ¬ IsSquare d) (hd': ¬ IsSquare d')
  ...
  have hQ := (Affine.Point.X_eq_iff (h₁ := mem_twist (ne_zero_of_not_square hd')
    ...
    (h₂ := Affine.nonsingular_scale hscale (mem_twist hd0
      (sqrtCandidate_twistRhs_sq hd0 h)))).mp hx
  -- In either case the u-coordinate of n·Q is s² times that of n·P
  have key: ∀ Q: (Twist d').toAffine.Point, Q = Affine.Point.scale hscale P ∨
      Q = -Affine.Point.scale hscale P → uCoord (n • Q) = s ^ 2 * uCoord (n • P) := by
    rintro Q (rfl | rfl)
...
/-- `_scalarMul` does not depend on the nonresidue it twists by: any two choices of `d` compute the
same function. -/
theorem _scalarMul_indep_d {d d': BaseField} (hd: ¬ IsSquare d) (hd': ¬ IsSquare d')
    (point scalar: Vector UInt8 32):
    _scalarMul d hd point scalar = _scalarMul d' hd' point scalar := by
  unfold _scalarMul
...
/-- In particular `scalarMul`, which twists by 2, is `_scalarMul` for every nonresidue. -/
theorem scalarMul_eq {d: BaseField} (hd: ¬ IsSquare d) (point scalar: Vector UInt8 32):
    scalarMul point scalar = _scalarMul d hd point scalar :=
  _scalarMul_indep_d two_not_square hd point scalar
