...
Bytes and 32-bit words are represented as `Nat`. The message is a vector of
bytes; the digest is a vector of eight 32-bit words, most significant first.
-/
...
def H0 : Vector Nat 8 := #v[
  0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
  0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
]

/-- Bitwise complement of a 32-bit word. -/
def not32 (a : Nat) : Nat := a ^^^ 0xffffffff

/-- `σ₀(x) = ROTR⁷(x) ⊕ ROTR¹⁸(x) ⊕ SHR³(x)` -/
def lowerSigma0 (x : Nat) : Nat := rotRight32 x 7 ^^^ rotRight32 x 18 ^^^ (x / 2 ^ 3)

/-- `σ₁(x) = ROTR¹⁷(x) ⊕ ROTR¹⁹(x) ⊕ SHR¹⁰(x)` -/
def lowerSigma1 (x : Nat) : Nat := rotRight32 x 17 ^^^ rotRight32 x 19 ^^^ (x / 2 ^ 10)

/-- `Σ₀(x) = ROTR²(x) ⊕ ROTR¹³(x) ⊕ ROTR²²(x)` -/
def upperSigma0 (x : Nat) : Nat := rotRight32 x 2 ^^^ rotRight32 x 13 ^^^ rotRight32 x 22

/-- `Σ₁(x) = ROTR⁶(x) ⊕ ROTR¹¹(x) ⊕ ROTR²⁵(x)` -/
def upperSigma1 (x : Nat) : Nat := rotRight32 x 6 ^^^ rotRight32 x 11 ^^^ rotRight32 x 25

/-- `Ch(e, f, g) = (e ∧ f) ⊕ (¬e ∧ g)` -/
def Ch (e f g : Nat) : Nat := (e &&& f) ^^^ (not32 e &&& g)

/-- `Maj(a, b, c) = (a ∧ b) ⊕ (a ∧ c) ⊕ (b ∧ c)` -/
def Maj (a b c : Nat) : Nat := (a &&& b) ^^^ (a &&& c) ^^^ (b &&& c)
...
def sha256Round (state : Vector Nat 8) (k w : Nat) : Vector Nat 8 :=
...
def messageSchedule (block : Vector Nat 16) : Vector Nat 64 :=
  let init : Vector Nat 64 := Vector.ofFn fun (i : Fin 64) =>
...
def pad {len : Nat} (msg : Vector Nat len) :
    Vector (Vector Nat 16) ((len + (55 + 64 - len % 64) % 64 + 9) / 64) :=
  let bitLen := len * 8
  let zeros := (55 + 64 - len % 64) % 64
  let numBlocks := (len + zeros + 9) / 64
  let totalLen := numBlocks * 64
  let padded : Vector Nat totalLen := Vector.ofFn fun (i : Fin totalLen) =>
    if h : i.val < len then
      msg[i.val]
...
def sha256 {len : Nat} (msg : Vector Nat len) : Vector Nat 8 :=
