...
def «pad10*1» x m : BitVec (padLen x m) :=
...
def squeeze (f : BitVec b → BitVec b) (r : Nat)
  ...
  else ((squeeze f r (f S) (d - r) hr) ++ S.extractLsb' 0 r).cast (by omega)
termination_by d

/-- SPONGE[f, pad10*1, r], FIPS 202 Algorithm 8. -/
def SPONGE {n : Nat} (f : BitVec b → BitVec b) (r : Nat)
    (N : BitVec n) (d : Nat) (hr : 0 < r ∧ r < b) : BitVec d :=
  let P := «pad10*1» r n ++ N
  let S := Fin.foldl ((n + padLen r n) / r) (fun S block =>
...
def KECCAK (c : Nat) (N : BitVec n) (d : Nat)
  ...
  SPONGE (Permutations.Keccak.keccak_f .w1600) (1600 - c) N d (by
