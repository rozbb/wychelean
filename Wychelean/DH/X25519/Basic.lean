...
import Wychelean.DH.X25519.Prime25519

open WeierstrassCurve

/-!
# X25519
This module contains an implementation of scalar multiplication in Curve25519. Key top-level
definitions are
* `scalarMulGeneric` — Scalar multiplication using the quadratic twist defined by parameter `d`
* `x25519` — Just `scalarMulGeneric` with `d = 2`
* `basepointMul` — Just `x25519` with the point set to the basepoint

We prove in `Wychelean.DH.X25519.TwistIndependence` that any appropriate choice of `d` in
`scalarMulGeneric` produces the same output. In other words: all twists are isomorphic, and the
choice of `d` does not affect the final u-coordinate when doing scalar multiplication
-/
...
/--
Curve25519 is the Montgomery curve Y² = x³ + 486662X² + X over `BaseField`. The Mathlib Weierstrass
equation is Y² + a₁XY + a₃Y = X³ + a₂X² + a₄X + a₆
-/
...
/-- Base point u-coordinate from <https://www.rfc-editor.org/rfc/rfc7748.html#section-4.1> -/
def BasePointU: BaseField := 9
/-- Base point v-coordinate from <https://www.rfc-editor.org/rfc/rfc7748.html#section-4.1> -/
...
/-- The basepoint, constructed as a proper affine point -/
...
the `x25519` function instantiates it at 2, and `X25519.TwistIndependence` shows the choice does not
matter.
-/

set_option maxRecDepth 4000 in
/--
2 is a nonresidue mod p. It is what the `x25519` function twists by, and it is also the fixed
nonresidue out of which `sqrtCandidate` builds a square root of -1, independently of the twist
parameter.
-/
...
/--
The quadratic twist d·Y² = X³ + 486662X² + X, put back into Weierstrass form by the invertible map
(X, Y) ↦ (X/d, Y/d²), which gives Y² = X³ + (d·486662)X² + d²X.
-/
...
/--
For any nonzero d—in particular for any nonresidue—the twist is elliptic too, and in a field a
nonzero discriminant is the same as a unit one.
-/
...
/--
decodeLittleEndian from <https://www.rfc-editor.org/rfc/rfc7748.html#section-5>. Except we read
_all_ bytes
-/
...
/--
decodeScalar25519 from <https://www.rfc-editor.org/rfc/rfc7748.html#section-5>. Clear the low 3 bits
of the first byte, clear the top bit of the last byte and set the second-highest, then decode
little-endian.
-/
...
/--
decodeUCoordinate from <https://www.rfc-editor.org/rfc/rfc7748.html#section-5>. With bits = 255 the
only unused bit is the top one of the last byte, which is masked off before decoding.
-/
...
/--
encodeUCoordinate from <https://www.rfc-editor.org/rfc/rfc7748.html#section-5>. Encodes a u
coordinate as little-endian bytes
-/
...
/-- If u satisfies the curve equation, then mkCurvePoint u is Some -/
...
/-- If u satisfies the twist equation, then mkTwistPoint u is Some -/
...
/- Every u-coordinate lies on the curve or on the twist, so `scalarMulGeneric` needs no fallback -/
...
/-
Returns the u-coordinate of the given point. The point at infinity maps to 0, per
...
/--
Multiplies a point by a scalar, in the argument order `X25519(k, u)` of the specification: the
scalar as a 32-byte little-endian representation of the integer, then the point as a 32-byte
little-endian representation of its u-coordinate. If the point is on the
curve, then multiplication is done on the curve. If it's on the twist by the nonresidue `d`, then
multiplication is done on that twist. The output is the 32-byte little-endian representation of the
resulting u-coordinate. Note that, per the specification, scalars are _clamped_, meaning some bits
are set/cleared before parsing as an integer.
-/
def scalarMulGeneric (d: BaseField) (hd: ¬ IsSquare d) (scalar point: Vector UInt8 32):
    Vector UInt8 32 :=
...
/--
`scalarMulGeneric` with the twisting parameter fixed to 2. By `scalarMulGeneric_indep_d` in
`Wychelean.DH.X25519.TwistIndependence` any other nonresidue gives the same function.
-/
def x25519 (scalar point: Vector UInt8 32): Vector UInt8 32 :=
  scalarMulGeneric 2 two_not_square scalar point

/--
Multiplies the Curve25519 basepoint by a scalar, given by a 32-byte little-endian representation of
the integer Note that, per the specification, scalars are _clamped_, meaning some bits are
set/cleared before parsing as an integer.
-/
def basepointMul (scalar: Vector UInt8 32): Vector UInt8 32 :=
  x25519 scalar (encodeUCoordinate BasePointU)
