...
def ofBitsLE (v : Vector Bool n) : BitVec n :=
  (ofBoolListLE v.toList).cast (by simp)
/-- Unpack a word, least significant bit first. -/
def toBitsLE (v : BitVec n) : Vector Bool n := Vector.ofFn fun i => v.getLsbD i

@[simp] theorem getLsbD_ofBitsLE (v : Vector Bool n) (i : Nat) (hi : i < n) :
    (ofBitsLE v).getLsbD i = v[i] := by
  simp only [ofBitsLE, getLsbD_cast, getLsbD_ofBoolListLE, List.getD_eq_getElem?_getD]
  simp [hi]

@[simp] theorem getElem_ofBitsLE (v : Vector Bool n) (i : Nat) (hi : i < n) :
    (ofBitsLE v)[i] = v[i] := getLsbD_ofBitsLE v i hi

@[simp] theorem toBitsLE_ofBitsLE (v : Vector Bool n) : toBitsLE (ofBitsLE v) = v := by
  apply Vector.ext
  intro i hi
  simp [toBitsLE]
...
@[simp] theorem bitsToBytes_bytesToBits (v : Vector UInt8 n) :
